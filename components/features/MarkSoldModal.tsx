...
export function MarkSoldModal({ item, isOpen, onClose }: { item: any, isOpen: boolean, onClose: () => void }) {
 ...
 <div className="bg-white rounded-[10px] w-full max-w-md overflow-hidden border border-slate-200 flex flex-col animate-in fade-in zoom-in-95 duration-200">
 ...
 <button onClick={onClose} className="text-slate-400 hover:text-slate-600 transition-colors p-1.5 rounded-[10px] hover:bg-slate-200">
 <X className="h-5 w-5" />
 </button>
 </div>
 ...
 <div className="bg-slate-50 p-4 rounded-[10px] border border-slate-200 flex justify-between items-center">
 <div>
 ...
 min="0"
 required
 className="w-full px-3 py-2.5 bg-white border border-slate-200 rounded-[10px] focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-slate-900 transition-colors "
 />
 </div>
 
 {error && (
 <p className="text-sm text-rose-600 bg-rose-50 p-2 rounded-[10px]">{error}</p>
 )}
 </div>
